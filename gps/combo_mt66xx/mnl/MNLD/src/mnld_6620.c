...
/*---------------------------------------------------------------------------*/
#if defined(ANDROID)
#define MND_MSG(fmt, ...) ALOGD("%s: " fmt, __FUNCTION__ ,##__VA_ARGS__)
#define MND_ERR(fmt, ...) ALOGE("%s: " fmt, __FUNCTION__ ,##__VA_ARGS__)
#define MND_TRC(fmt, ...) ALOGD("%s: " fmt, __FUNCTION__ ,##__VA_ARGS__)
...
static int launch_daemon_thread(void) 
{
    ...
    if ((pid = fork()) < 0) {
		...
		// For permission reason, use prop of "persist.radio.mediatek.chipid"
		while(get_time-- != 0 && (property_get("persist.radio.mediatek.chipid", chip_id, NULL) <= 0))
		{
			usleep(100000);
		}
    ...
    *eof = 0;
    ...
    if(get_prop()){
        ...
        if (write_gps_log_to_sdcard(str) < 0)
            MND_ERR("/sdcard/GPS.LOG write start fail");
        MND_MSG("write done");      
    }      
        return status_ret;
    ...
    if(get_prop()){
        ...
        MND_MSG("write begin: %s", str);
        if (write_gps_log_to_sdcard(str) < 0)
            MND_ERR("/sdcard/GPS.LOG write stop fail");
        MND_MSG("write done");
    }
