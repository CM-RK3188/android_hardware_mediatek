...
public class FMRadioService extends Service implements FMRecorder.OnRecorderStateChangedListener {
    ...
    public void powerUpAsync(float frequency) {
        ...
        LogUtils.d(TAG, "ddb dbg: POWERUP, mFmServiceHandler:" + (mFmServiceHandler==null));
        mFmServiceHandler.removeMessages(FMRadioListener.MSGID_POWERUP_FINISHED);
        LogUtils.d(TAG, "ddb dbg: POWERDOWN, mFmServiceHandler:" + (mFmServiceHandler==null));
        mFmServiceHandler.removeMessages(FMRadioListener.MSGID_POWERDOWN_FINISHED);
        LogUtils.d(TAG, "ddb dbg: after all, mFmServiceHandler:" + (mFmServiceHandler==null));
    ...
    private void sendBroadcastToStopOtherAPP() {
        ...
        if (FMRadioFeatureOption.MTK_MT519X_FM_SUPPORT) {
    ...
    private boolean tuneStation(float frequency) {
        LogUtils.d(TAG, ">>> VER:201304240130 FMRadioService.tune: " + frequency);
        if (mIsPowerUp) {
            setRDS(false);
...
//            enableFMAudio(true);
    ...
    public void onCreate() {
        ...
        LogUtils.d(TAG, "ddb dbg: before new, mFmServiceHandler:" + (mFmServiceHandler==null));
        mFmServiceHandler = new FmRadioServiceHandler(handlerThread.getLooper());
        LogUtils.d(TAG, "ddb dbg: after new, mFmServiceHandler:" + (mFmServiceHandler==null));
    ...
    public boolean requestAudioFocus() {
        ...
        int audioFocus = mAudioManager.requestAudioFocus(mAudioFocusChangeListener, AudioManager.STREAM_MUSIC,
