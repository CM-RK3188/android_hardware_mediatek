...
public class BT_Service extends Service {
	...
	public void onCreate() {
		...
		filter.addAction(Basic_Combo_Tool.BT_BROADCAST);
	...
	public class mMessageReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				...
				if (Basic_Combo_Tool.BT_BROADCAST.equals(action)) {
					Bundle bunde = intent.getExtras();
					...
					if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
						...
						short rssi = (short) (intent.getExtras().getShort(
								BluetoothDevice.EXTRA_RSSI) - 255);
						int device_class = device.getBluetoothClass()
								.getMajorDeviceClass();
						...
						btservice_broadcast("MSG", "find device: \n"
								...
								+ Short.toString(rssi) + "\n"
								+ string_device_class, "null");
						btservice_broadcast("BT_inquiry", device.getName(),
								device.getAddress() + "\n"
										+ Integer.toString(rssi) + "dbm",
								Integer.toString(rssi), string_device_class);
