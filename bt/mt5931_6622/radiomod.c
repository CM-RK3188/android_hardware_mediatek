...
static int addr_from_vflash(unsigned char *addr)
{
    int fd;
    char bd_addr[6] = {0};

	LOG_DBG("addr_from_vflash.\n");
    fd = open("/dev/vflash", O_RDWR|O_EXCL);
    if(fd < 0) {
        LOG_ERR("Can't open vflash! %s\n", strerror(errno));
        return -1;
    }

    if(ioctl(fd, READ_BDADDR_FROM_FLASH, (unsigned long)bd_addr) < 0) {
        LOG_ERR("vflash ioctl failed\n");
        close(fd);
        return -1;
    }
	...
	LOG_DBG("vflash: %02x:%02x:%02x:%02x:%02x:%02x\n", 
...
static int addr_from_wifi_mac(unsigned char *addr) 
{
	FILE *fd;
	unsigned char wifi_mac[32];
	char bd_addr[6] = {0};
	
	LOG_DBG("addr_from_wifi_mac.\n");

	fd = fopen("/data/bluetooth/wlan_mac", "r");
	if(fd == NULL) {
        LOG_ERR("Can't open /data/bluetooth/wlan_mac! %s\n", strerror(errno));
        return -1;
    }
	fread(wifi_mac, 1, 18, fd);
	fclose(fd);

	sscanf(wifi_mac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
		&bd_addr[0], &bd_addr[1], &bd_addr[2], &bd_addr[3], &bd_addr[4], &bd_addr[5]);
	bd_addr[5] += 2;
        bd_addr[5] &= ~1;
	
	LOG_DBG("wifi_mac: %02x:%02x:%02x:%02x:%02x:%02x\n", 
		bd_addr[0], bd_addr[1], bd_addr[2], bd_addr[3], bd_addr[4], bd_addr[5]);
	memcpy(addr, bd_addr, 6);
	return 0;
}

#define BD_ADDR_AUTOGEN
static BOOL GORMcmd_HCC_Set_Local_BD_Addr(HC_BT_HDR *p_cmd)
{
    ...
    if (fInternalCfg)
    {
        ...
        if (0 == memcmp(btinit->bt_conf.fields.addr, sDefaultCfg.addr, 6))
        {
			...
			if(BT_ADDR_GENERATE_METHOD == 1) {
				if(addr_from_vflash(btinit->bt_conf.fields.addr) != 0) {
					GetRandomValue(btinit->bt_conf.fields.addr);
				}
			}

			if(BT_ADDR_GENERATE_METHOD == 2) {
				if(addr_from_wifi_mac(btinit->bt_conf.fields.addr) != 0) {
					GetRandomValue(btinit->bt_conf.fields.addr);
				}
			}
			
			if(BT_ADDR_GENERATE_METHOD == 0) {
            	GetRandomValue(btinit->bt_conf.fields.addr);
			}
